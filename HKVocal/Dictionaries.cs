...
public static class Dictionaries
{
    ...
    public static readonly Dictionary<(string, string, string), Action<PlayMakerFSM>> SceneFSMEdits =
        new Dictionary<(string, string, string), Action<PlayMakerFSM>>()
        {
            {
                ("GG_Radiance", "Boss Control", "Control"), fsm =>
                {
                    if (BossSequenceController.IsInSequence)
                        fsm.AddAction("Flash Down",
                            new DreamDialogueAction("RADIANCE_1", "Enemy Dreams") { waitTime = 5 });
                }
            },
            {
                ("Dream_Final_Boss", "Boss Control", "Control"), fsm =>
                {
                    if (BossSequenceController.IsInSequence)
                        fsm.AddAction("Flash Down",
                            new DreamDialogueAction("RADIANCE_1", "Enemy Dreams") { waitTime = 5 });
    ...
    public static readonly Dictionary<(string, string), Action<PlayMakerFSM>> GoFSMEdits =
        new Dictionary<(string, string), Action<PlayMakerFSM>>()
        {
            { ("DialogueManager", "Box Open Dream"), FSMEdits.BoxOpenDream },
            { ("Absolute Radiance", "Control"), FSMEdits.RadianceControl },
            {
                ("Absolute Radiance", "Phase Control"), fsm =>
                {
                    if (BossSequenceController.IsInSequence)
            ...
            {
                ("Radiance", "Phase Control"),
                fsm => fsm.AddAction("Set Phase 2", new DreamDialogueAction("RADIANCE_2", "Enemy Dreams"))
            },
            ...
            {
                ("Mato", "nailmaster"), fsm => fsm.AddMethod("Death Start", () =>
                {
                    if (HKVocals.instance.audioSource.clip.name.Contains("MATO")) HKVocals.instance.audioSource.Stop();
                })
            },
    ...
    public static readonly Dictionary<string, Action<PlayMakerFSM>> FSMChanges =
        new Dictionary<string, Action<PlayMakerFSM>>()
        {
